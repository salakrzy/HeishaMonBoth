...
void setupModbus(){  // ***********************************************************************************************
  ...
  //  Serial.end();
   // RTUutils::prepareHardwareSerial(Serial);
    ...
    ModbGate.setTimeout(2000);			// Set message timeout value in milliseconds, how long we wait for device answer
    ModbGate.begin(swSerial,9600,-1,100);	//  ModbGate.begin(Serial,1);	Start ModbusRTU background task on CPU number 1
    ...
    actModbDev=0;
        Serial.updateBaudRate(115200); //Set up Serial port for better monitor speed
  } else{
      Serial.updateBaudRate(115200); //Set up Serial port for better monitor speed
