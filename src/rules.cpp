...
bool send_command(byte* command, int length, int source);//####ESP32
    ...
    if(parsing == 0 && !heishamonSettings.listenonly) {
      ...
      for(uint8_t x = 0; x < sizeof(commands) / sizeof(commands[0]); x++) {
        ...
        if(stricmp((char *)&var->token[1], tmp.name) == 0) {
          ...
          log_message(log_msg);
          send_command(cmd, len,CZTAW_NOT);   ////ESP32 rimi CZTAW__NOT =0 means do not forward  to CZTAW Pump answer for this request
          break;
        }
      }
