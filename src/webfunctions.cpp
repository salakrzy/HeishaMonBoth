    ...
    if (LittleFS.exists("/config.json")) {
      ...
      if (configFile) {
        ...
        if (!error) {
          ...
          heishamonSettings->logSerial = ( jsonDoc["logSerial"] == "enabled" ) ? true : false;
...
void settingsToJson(DynamicJsonDocument &jsonDoc, settingsStruct *heishamonSettings) {
  ...
  if (heishamonSettings->logSerial) {
    jsonDoc["logSerial"] = "enabled";
  } else {
    jsonDoc["logSerial"] = "disabled";
  }
...
int saveSettings(struct webserver_t *client, settingsStruct *heishamonSettings) {
  ...
  while (tmp) {
    ...
    } else if (strcmp(tmp->name.c_str(), "logSerial") == 0) {
      jsonDoc["logSerial"] = tmp->value;
...
int getSettings(struct webserver_t *client, settingsStruct *heishamonSettings) {
  switch (client->content) {
    ...
    case 7: {
        char str[20];
        webserver_send_content_P(client, PSTR(",\"logSerial\":"), 13);
