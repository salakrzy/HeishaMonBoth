    ...
    if(parsing == 0 && !heishamonSettings.listenonly) {
      ...
      for(uint8_t x = 0; x < sizeof(commands) / sizeof(commands[0]); x++) {
        ...
        if(strcmp((char *)&var->token[1], tmp.name) == 0) {
          ...
          log_message(log_msg);
          send_command(cmd, len,CZTAW_NOT);   // rimi CZTAW__NOT =0 means do not forward  to CZTAW Pump answer for this request
          break;
        }
      }
  ...
  for(i=0;i<nrrules;i++) {
    ...
    if(rules[i]->ast.buffer[start->go] == TEVENT) {
      ...
      if(
        ...
        logprintf("%s%d %s %d %s", "rule #", rules[i]->nr, "was executed in", rules[i]->timestamp.second - rules[i]->timestamp.first, "microseconds");
